...
<html lang="en">
        ...
        <div id="layoutSidenav">
            ...
            <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Purchase Return</h1>
    ...
    <table id="export" class="table table-striped" style="width:100%">
        ...
        <tbody>
            ...
            include "koneksi/koneksi.php";
            $no = 1;
            $tampil = mysqli_query($koneksi, "SELECT * FROM purchase_return ORDER BY id_purchase_return DESC");
            while ($data = mysqli_fetch_array($tampil)) {
                echo "<tr>
                <td>".$no."</td>
                <td>".$data['id_purchase_order']."</td>
                <td>".$data['id_material']."</td>
                <td>".$data['date_purchase_return']."</td>
                <td>".$data['total_material']."</td>
                <td>".$data['total_price']."</td>
                <td>".$data['payment_method']."</td>
                <td>".$data['description']."</td>
                <td>".$data['reason_for_return']."</td>
                <td>".$data['status']."</td>
                <td>
                ...
                <div class='modal fade' id='modalubah".$no."' data-bs-backdrop='static' data-bs-keyboard='false' tabindex='-1' aria-labelledby='staticBackdropLabel' aria-hidden='true'>
                    <div class='modal-dialog'>
                        <div class='modal-content'>
                            ...
                            <form method='POST' action='aksi/aksi_purchasereturn.php'>
                                <input type='hidden' name='id_purchase_return' value='".$data['id_purchase_return']."'>
                                <div class='modal-body'>
                                    <div class='mb-3'>
                                        <label class='form-label'>ID Purchase order</label>
                                        <input type='number' class='form-control' name='id_purchase_order' value='".$data['id_purchase_order']."' placeholder='Masukkan Tanggal Mulai'>
                                    </div>
                                     <div class='mb-3'>
                                        <label class='form-label'>ID Material</label>
                                        <input type='number' class='form-control' name='id_material' value='".$data['id_material']."' placeholder='Masukkan Nama Produk'>
                                    </div>
                                    <div class='mb-3'>
                                        <label class='form-label'>Date Purchase Return</label>
                                        <input type='date' class='form-control' name='date_purchase_return' value='".$data['date_purchase_return']."' placeholder='Masukkan Tanggal Mulai'>
                                    </div>
                                    <div class='mb-3'>
                                        <label class='form-label'>Total Material</label>
                                        <input type='number' class='form-control' name='total_material' value='".$data['total_material']."' placeholder='Masukkan Tanggal Mulai'>
                                    </div>
                                    <div class='mb-3'>
                                        <label class='form-label'>Total Price</label>
                                        <input type='number' class='form-control' name='total_price' value='".$data['total_price']."' placeholder='Masukkan Tanggal Mulai'>
                                    </div>
                                    <div class='mb-3'>
                                        <label class='form-label'>Payment Method</label>
                                        <select class='form-select' name='payment_method'>
                                            <option value='E-Money' ".(($data['payment_method'] == 'E-Money') ? 'selected' : '').">E-Money</option>
                                            <option value='Bank' ".(($data['payment_method'] == 'Bank') ? 'selected' : '').">Bank</option>
                                        </select>
                                    </div>
                                    <div class='mb-3'>
                                        <label class='form-label'>Description</label>
                                        <input type='text' class='form-control' name='description' value='".$data['description']."' placeholder='Masukkan Tanggal Mulai'>
                                    </div>
                                    <div class='mb-3'>
                                        <label class='form-label'>Reason For Return</label>
                                        <input type='text' class='form-control' name='reason_for_return' value='".$data['reason_for_return']."' placeholder='Masukkan Tanggal Mulai'>
                                    </div>
                                    <div class='mb-3'>
                            ...
                            <form method='POST' action='aksi/aksi_purchasereturn.php'>
                                ...
                                <div class='modal-body'>
                                    <h5> Dengan menghapus data ini anda akan menghapus data dari daftar!<br><br>
                                        <span class='text-danger'>".$data['id_purchase_return']."</span>
                                    </h5>
                                </div>
                                <div class='modal-footer'>
                        ...
                        <div class="modal-content">
                            ...
                            <form method="POST" action="aksi/aksi_purchasereturn.php">
                                <div class="modal-body">
                                    <div class="mb-3">
                                        <label class="form-label">ID Purchase Order</label>
                                        <input type="number" class="form-control" name="id_purchase_order" placeholder="Masukkan ID Sales Order">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">ID Material</label>
                                        <input type="number" class="form-control" name="id_material" placeholder="Masukkan Tanggal Pengiriman">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Date Purchase Return</label>
                                        <input type="date" class="form-control" name="date_purchase_return" placeholder="Masukkan ID Sales Order">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Total Material</label>
                                        <input type="number" class="form-control" name="total_material" placeholder="Masukkan ID Sales Order">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Total Price</label>
                                        <input type="number" class="form-control" name="total_price" placeholder="Masukkan ID Sales Order">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Payment Method</label>
                                        <select class="form-select" name="payment_method">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Description</label>
                                        <input type="text" class="form-control" name="description" placeholder="Masukkan ID Sales Order">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Reason For Return</label>
                                        <input type="text" class="form-control" name="reason_for_return" placeholder="Masukkan ID Sales Order">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Status</label>
                                        <select class="form-select" name="status">
